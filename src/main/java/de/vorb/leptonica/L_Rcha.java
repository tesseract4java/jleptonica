package de.vorb.leptonica;
import com.sun.jna.Structure;
import java.util.Arrays;
import java.util.List;
/**
 * Data returned from correlation matching on an array of characters<br>
 * <i>native declaration : C:\Users\Paul\Studium\Masterarbeit\Entwicklung\VS2013\tesseract-vs2013\liblept\include\recog.h:187</i><br>
 * This file was autogenerated by <a href="http://jnaerator.googlecode.com/">JNAerator</a>,<br>
 * a tool written by <a href="http://ochafik.com/">Olivier Chafik</a> that <a href="http://code.google.com/p/jnaerator/wiki/CreditsAndLicense">uses a few opensource projects.</a>.<br>
 * For help, please visit <a href="http://nativelibs4java.googlecode.com/">NativeLibs4Java</a> , <a href="http://rococoa.dev.java.net/">Rococoa</a>, or <a href="http://jna.dev.java.net/">JNA</a>.
 */
public class L_Rcha extends Structure {
	/**
	 * indices of best templates<br>
	 * C type : Numa*
	 */
	public de.vorb.leptonica.Numa.ByReference naindex;
	/**
	 * correlation scores of best templates<br>
	 * C type : Numa*
	 */
	public de.vorb.leptonica.Numa.ByReference nascore;
	/**
	 * character strings of best templates<br>
	 * C type : Sarray*
	 */
	public de.vorb.leptonica.Sarray.ByReference satext;
	/**
	 * indices of best samples<br>
	 * C type : Numa*
	 */
	public de.vorb.leptonica.Numa.ByReference nasample;
	/**
	 * x-locations of templates (delx + shiftx)<br>
	 * C type : Numa*
	 */
	public de.vorb.leptonica.Numa.ByReference naxloc;
	/**
	 * y-locations of templates (dely + shifty)<br>
	 * C type : Numa*
	 */
	public de.vorb.leptonica.Numa.ByReference nayloc;
	/**
	 * widths of best templates<br>
	 * C type : Numa*
	 */
	public de.vorb.leptonica.Numa.ByReference nawidth;
	public L_Rcha() {
		super();
	}
	protected List<? > getFieldOrder() {
		return Arrays.asList("naindex", "nascore", "satext", "nasample", "naxloc", "nayloc", "nawidth");
	}
	/**
	 * @param naindex indices of best templates<br>
	 * C type : Numa*<br>
	 * @param nascore correlation scores of best templates<br>
	 * C type : Numa*<br>
	 * @param satext character strings of best templates<br>
	 * C type : Sarray*<br>
	 * @param nasample indices of best samples<br>
	 * C type : Numa*<br>
	 * @param naxloc x-locations of templates (delx + shiftx)<br>
	 * C type : Numa*<br>
	 * @param nayloc y-locations of templates (dely + shifty)<br>
	 * C type : Numa*<br>
	 * @param nawidth widths of best templates<br>
	 * C type : Numa*
	 */
	public L_Rcha(de.vorb.leptonica.Numa.ByReference naindex, de.vorb.leptonica.Numa.ByReference nascore, de.vorb.leptonica.Sarray.ByReference satext, de.vorb.leptonica.Numa.ByReference nasample, de.vorb.leptonica.Numa.ByReference naxloc, de.vorb.leptonica.Numa.ByReference nayloc, de.vorb.leptonica.Numa.ByReference nawidth) {
		super();
		this.naindex = naindex;
		this.nascore = nascore;
		this.satext = satext;
		this.nasample = nasample;
		this.naxloc = naxloc;
		this.nayloc = nayloc;
		this.nawidth = nawidth;
	}
	public static class ByReference extends L_Rcha implements Structure.ByReference {
		
	};
	public static class ByValue extends L_Rcha implements Structure.ByValue {
		
	};
}
